...
import { getMonitorData } from '../../other/commone';
...
const Monitor = () => {
    ...
    useEffect(() => {
        ...
        if(cartNumb.globalObj.monitorData===0){
            getMonitorData().then((data) => {
                cartNumb.setGlobalData("monitorData",data)
                //setMonitorData(data);
            });
        }
    ...
    const increMent = (index) => {
        // setQty(qty+1);
        console.log(index, monitorData[index]);
        setMonitorData(prevState => {
            let updateMonitorData = prevState.map((item, ind) => {
                if (ind === index) {
    ...
    return (
        <div className='container'>
            <div className='row'>
                {cartNumb.globalObj.monitorData.length>0&&cartNumb.globalObj.monitorData.map((item,index)=>{
